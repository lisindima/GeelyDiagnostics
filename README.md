...
- **Диагностика** — DTC, сгруппированные по ECU; несколько ошибок одного блока
  показываются отдельными строками. Дополнительно доступны PartInfo, сведения о
  наличии диагностических API и кадры OBD2 при поддержке со стороны ГУ;
...
При совпадении **одного signalId** сначала применяется явный нормализующий AOSP
mapping; при его отсутствии — mapping выбранного профиля. Если оба отсутствуют,
...
приоритет такой: выбранный профиль VHAL → AOSP VHAL → ECARX → raw.
...
Экспорт JSON версии схемы `8` сохраняет типизированные `normalizedValue`, единицы,
