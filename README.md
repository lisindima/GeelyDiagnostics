...
системный выбор файла, отчёт безопасно сохраняется в каталоге приложения.
...
этом режиме. Карточки показывают время последнего получения значения; непрерывные
...
или смене профиля. Если callback API недоступен на конкретной ГУ, значения остаются из
