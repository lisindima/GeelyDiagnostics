...
internal fun LazyListScope.ecuDiagnosticsItems(state: DiagnosticsUiState) {
    ...
    if (state.dtcs.isEmpty()) {
        item("ecu-empty") { EmptyMessage("Данные по блокам пока не получены.") }
    } else {
        dtcTableItems(state.dtcs)
    }
}

private fun LazyListScope.dtcTableItems(dtcs: List<DtcRecord>) {
    ...
    if (noCodesReturned) {
        item("ecu-no-codes") {
            Card(
                ...
                Text(
                    text = "Блоки ответили без DTC-кодов. Это не подтверждает отсутствие неисправностей: семантика статуса vendor API не документирована.",
    ...
    items(recordsByEcu.entries.toList(), key = { "ecu-${it.key}" }) { (ecuType, records) ->
